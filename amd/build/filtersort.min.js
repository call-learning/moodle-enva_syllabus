...
/**
   ...
   */Object.defineProperty(_exports,"__esModule",{value:!0}),_exports.init=void 0,_config=_interopRequireDefault(_config),_jquery=_interopRequireDefault(_jquery);_exports.init=function(filterItemId){var formNode=document.querySelector("#"+filterItemId+" form");formNode.addEventListener("submit",(function(e){e.preventDefault();var filterinfo=getFilterData(formNode,!1);if(filterinfo){var event=new CustomEvent("enva-syllabus-catalog-filter",{detail:filterinfo});document.dispatchEvent(event)}}))};var getFilterData=function(target,ignoresesskey){var data=(0,_jquery.default)(target).serializeArray(),sesskeyconfirmed=!1,filterarray=[],sortarray=[];data.forEach((function(d){if("sesskey"===d.name)sesskeyconfirmed=d.value===_config.default.sesskey;else if(d.value){var filtername=d.name.match(/^filter_(.+)/);filtername&&filterarray.push({fieldsn:filtername[1],value:d.value})}}));var sortfield=data.find((function(d){return"sort"===d.name}));if(sortfield&&sortfield.value){var sortfieldspec=sortfield.value.match(/^(.+)-(.+)/);sortfieldspec&&sortarray.push({order:sortfieldspec[2],field:sortfieldspec[1]})}return sesskeyconfirmed||ignoresesskey?{filters:filterarray,sorts:sortarray}:null}}));
